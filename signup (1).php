...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['signup'])) {
        ...
        try {
            $mail->isSMTP();
            $mail->Host = 'mail.delishgofoods.com'; // Truehost mail server
            $mail->SMTPAuth = true;
            $mail->Username = 'verify@delishgofoods.com'; // your domain email
            $mail->Password = 'verify752nL'; // your email password
            $mail->SMTPSecure = 'ssl'; // or 'tls' if needed
            $mail->Port = 465; // or 587 for 'tls'

            $mail->setFrom('verify@delishgofoods.com', 'DelishGo');
            $mail->addAddress($email);
            $mail->isHTML(true);
            $mail->Subject = 'DelishGo Email Verification';
            $mail->Body = "<p>Hello $name,</p><p>Your OTP code is: <strong>$otp</strong></p><p>Use it to verify your account on DelishGo.</p>";
            $mail->AltBody = "Hello $name,\nYour OTP code is: $otp\nUse it to verify your account on DelishGo.";

            $mail->send();
            $_SESSION['popup_message'] = "OTP sent to $email. Please verify.";
            $_SESSION['redirect_url'] = "verify.php";
        } catch (Exception $e) {
            $_SESSION['popup_message'] = "OTP could not be sent. Mailer Error: {$mail->ErrorInfo}";
...
<body>
  ...
  <main id="signup" class="auth-container">
